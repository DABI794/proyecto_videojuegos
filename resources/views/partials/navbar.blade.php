<nav class="fixed top-0 left-0 right-0 z-40 bg-[#0f172a]/95 backdrop-blur border-b border-[#334155]">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
            ...
            {{-- Logo --}}
            <a href="{{ route('home') }}" class="flex items-center gap-2 text-white font-bold text-lg no-underline hover:opacity-80 transition-opacity">
            ...
            <div class="flex items-center gap-3">
                ...
                <form action="{{ route('products.index') }}" method="GET" class="hidden sm:flex">
                    <div class="relative">
                        ...
                        >
                        <button type="submit" class="absolute right-2 top-1/2 -translate-y-1/2 text-[#64748b] hover:text-[#6366f1] transition-colors bg-transparent border-0">
                ...
                @auth
                    {{-- Carrito --}}
                    <a href="{{ route('cart.index') }}" class="relative p-2 text-[#94a3b8] hover:text-[#f1f5f9] transition-colors no-underline">
                ...
                @else
                    ...
                    <a href="{{ route('register') }}" class="text-sm bg-[#6366f1] hover:bg-[#4f46e5] text-white px-4 py-2 rounded-xl no-underline transition-colors font-medium">
                        Registrarse
                    </a>
                @endauth
        ...
        <div id="mobile-menu" class="hidden md:hidden border-t border-[#334155] py-3 space-y-1">
            <a href="{{ route('home') }}" class="block px-3 py-2 text-sm text-[#94a3b8] hover:text-[#f1f5f9] no-underline">Inicio</a>
            <a href="{{ route('products.index') }}" class="block px-3 py-2 text-sm text-[#94a3b8] hover:text-[#f1f5f9] no-underline">Productos</a>
            <form action="{{ route('products.index') }}" method="GET" class="px-3 py-2">
                <input type="text" name="buscar" placeholder="Buscar juegos..." class="w-full bg-[#1e293b] border border-[#334155] text-[#f1f5f9] text-sm rounded-xl px-3 py-2 focus:outline-none focus:border-[#6366f1]">
            </form>
        </div>
    </div>
</nav>
